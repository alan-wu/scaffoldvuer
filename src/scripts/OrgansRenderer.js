...
const OrgansViewer = function (ModelsLoaderIn) {
  ...
  const preRenderTimeUpdate = function () {
    ...
    if (_this.sceneData.nerveMap && _this.sceneData.nerveMap.additionalReader)
      _this.sceneData.nerveMap.additionalReader.setTime(currentTime / duration);
    ...
    if (intersected !== undefined) {
      ...
      if (intersectedObject) {
        if (intersectedObject.name) {
          ...
          const annotations = _this.getAnnotationsFromObjects([intersectedObject], undefined);
      ...
      if (idObject.id) {
        ...
        if (idObject.object.userData.isGlyph) {
          if (idObject.object.name) {
            ...
            _this.setSelectedByZincObjects(
              idObject.object.userData.getGlyphset(),
              coords,
              extraData,
              true,
            );
  ...
  this.updateFieldvisibility = function (dataFields, value) {
    ...
    if (value > -1) {
      ...
      if (
        _this.scene.findGeometriesWithGroupName(partName).length > 0 ||
        ...
        if (partDetails != undefined) {
          _this.scene.loadMetadataURL(
            modelsLoader.getOrgansDirectoryPrefix() + '/' + partDetails.meta,
          );
        }
      }
    }
  ...
  this.loadOrgansFromURL = function (
    url,
    speciesName,
    systemName,
    partName,
    ...
    if (_this.zincRenderer) {
      if (partName && _this.sceneData.metaURL !== url) {
        ...
        organScene.addZincObjectRemovedCallbacks(
          _removeOrganPartCallback(undefined, partName, false),
        );
        organScene.loadMetadataURL(
          url,
          singleItemFinishCallback(),
